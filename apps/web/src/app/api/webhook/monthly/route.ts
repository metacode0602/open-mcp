...
import { apps,rankingRecords, rankings, repos } from "@repo/db/schema";
...
const MonthlyRankingSchema = z.object({
  year: z.number(),
  month: z.number(),
...
export async function POST(request: NextRequest) {
  try {
    ...
    // 验证请求数据
    const validatedData = MonthlyRankingSchema.parse(body);
    const { year, month, projects, total_projects } = validatedData;

    // 生成periodKey，格式为 "YYYY-MM"
    ...
    const [ranking] = await db
      .insert(rankings)
      .values({
        name: `GitHub Monthly Ranking ${periodKey}`,
        source: "github",
        description: `GitHub monthly trending repositories for ${periodKey}`,
    ...
    return NextResponse.json({
      success: true,
      message: "Monthly ranking data saved successfully",
      data: {
        rankingId,
        periodKey,
    ...
    console.error("Error processing monthly ranking data:", error);
